...
const GREETINGS: Record<TimeSlot, string[]> = {
  // 아침 (06:00 ~ 11:00)
  morning: [
    '{nickname}님, 좋은 아침이에요! 오늘 하루를 함께 시작할 영화 한 편 어때요?',
    '{nickname}님, 모닝커피와 함께 가벼운 영화 어떠세요?',
    '{nickname}님, 상쾌한 아침이에요! 기분 좋은 영화로 시작해볼까요?',
    '{nickname}님, 아침부터 영화라니 여유로운 하루군요!',
    '{nickname}님의 오늘 하루 에너지가 될 영화를 찾아드릴게요',
    '{nickname}님, 햇살 좋은 아침이에요! 어떤 영화가 끌리세요?',
  ...
  lunch: [
    ...
    '{nickname}님, 나른한 점심이에요! 영화로 채워볼까요?',
  ...
  evening: [
    ...
    '{nickname}님, 굿이브닝! 오늘 밤의 영화를 골라드릴게요',
  ...
  dawn: [
    ...
    '{nickname}님, 모두 잠든 시간이에요! 당신만의 영화 타임이에요',
 ...
 * @returns 포맷팅된 인사말 문자열
 */
export function getGreeting(nickname?: string): string {
  const timeSlot = getCurrentTimeSlot();

  if (!nickname) {
    return getRandomItem(DEFAULT_GREETINGS[timeSlot]);
  }

  const template = getRandomItem(GREETINGS[timeSlot]);
